...
async function main() {
  ...
  if (wantFetch) {
    await updateOldRepositories({db, rootDir, autoYes});
  }
...
async function checkBitbucketAndClone({db, rootDir, autoYes}) {
  ...
  const projects = await getProject({jar, domain}); // Return [{url, projectName}]
  console.log(`Found ${projects.length} projects`);

  let repositaryStats = {};

  for (let project of _.map(projects)) {
    const repositories = await getRepositaries({project, jar, domain}); // Return [{url, repoName}]
...
async function updateOldRepositories({db, rootDir, autoYes}) {
