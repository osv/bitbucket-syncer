...
async function main() {
  ...
  if (wantStats || wantFetch || wantCheckBitbucket) {
    await printStats({db});
  }
}
    ...
    for (let repository of getDirectories(projectPath)) {
      ...
      if (fs.existsSync(path.join(repoPath, '.git'))) {
        ...
        try {
          ...
          console.log(`Failed to fetch repo ${rootDir}, you might want to remove this directory or add to ignore list`);
...
async function printStats({db}) {
  ...
  const stats = _.chain(projects)
          ...
          .flatten()
          .map(it => {
            it.updated = it.updated ? moment(it.updated).fromNow() : '';
            return it;
          })
          .map(it => {
